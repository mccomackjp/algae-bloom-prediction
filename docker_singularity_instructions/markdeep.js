...
var STYLESHEET = entag('style',
    ...
    '.md pre.listing {tab-size:4;-moz-tab-size:4;-o-tab-size:4;overflow:auto}' +
    ...
    '.md div.minus blockquote, .md div.bonus blockquote{' +
    'position:relative;' +
    'margin:1em 0;' +
    'padding:.4rem 1rem;' +
    'border-radius:.2rem;' +
    ...
    '@media screen and (max-width: 960px){body{font-size:120%}}' +
    '@media screen and (max-width: 480px){body{font-size:140%}}' +
    ...
    '.md .mediumTOC{float:right;font-size:75%;line-height:15px;border-left:1px solid #CCC;padding-left:15px;margin:15px 0px 15px 25px}' +

    '.md .mediumTOC .level1{font-weight:600}' +

    '.md .longTOC .level1{font-weight:600;display:block;padding-top:12px;margin:0 0 -20px}' +
     
    '.md .shortTOC{text-align:center;font-weight:bold;margin-top:15px;font-size:90%}' +
    ...
    '.md .admonition.bonus>.admonition-title{' +
    ...
    '.md .admonition.minus>.admonition-title{' +
...
function replaceLists(s, protect) {
    ...
    // Blank line or line ending in colon, line that starts with #., *, +, -, or checkbox/uncheckedbox
...
/** Inserts a table of contents for grading comments. */
function insertGrading(s, protect, score, possible) {

    // Place score at beginning of webpage title
    s = s.rp(/<title>(.*?)<\/title>/gi, function (title, text) {
...
function insertTableOfContents(s, protect) {
    ...
    var AFTER_METRICS = '<div class=afterMetrics><\/div>';
    var afterMetricsLocation = s.indexOf(AFTER_METRICS);
    if (afterMetricsLocation === -1) {
        afterMetricsLocation = 0;
    } else {
        afterMetricsLocation += AFTER_METRICS.length;
    }

    if(afterMetricsLocation>0) {
        insertLocation = afterMetricsLocation;
    }
...
function getEarned(str) {
    ...
    if(!!matches) {
        earned = matches[0].split('\n')
            ...
            .map(r => r.replace(/.* (.*[0-9]).* \|.* .*[0-9].*$/ig, function(row, points) {
                return points;
            }))
...
function markdeepToHTML(str, elementMode) {
    ...
    str = str.rp(/^!!![ \t]*metrics\:?(.*)\n([ \t]{3,}.*\s*\n)*/igm, function (match, title) {
        // Have to extract the body by splitting match because the regex doesn't capture the body correctly in the multi-line case
        match = match.trim();
	if(title === '') { title='Instructor Feedback'; }
        return '\n\n' + entag('div', (entag('div', title, protect('class="admonition-title"')) + '\n' + match.ss(match.indexOf('\n'))).trim(), protect('class="admonition metrics"')) + '<div class=afterMetrics></div>';
    });
    ...
    // REFERENCE-LINKS: [foo]: http://foo.com
    ...
    // Add grading TOC
    if(gradingOn) {
        str = insertGrading(str, protect, score, possible); // CCT
    }
...
function diagramToSVG(diagramString, alignmentHint) {
    ...
    var svg = '<div class="svgdiagram"><svg class="diagram" xmlns="http://www.w3.org/2000/svg" version="1.1" height="' + 
        ...
        for (var y = 0; y < grid.height; ++y) {
            for (var x = 0; x < grid.width; ++x) {
                ...
                if (/[\u2B22\u2B21]/.test(c)) {
                    ...
                    svg += '<text text-anchor="middle" x="' + (x * SCALE) + '" y="' + (4 + y * SCALE * ASPECT) + '" style="font-size:130%">' + escapeHTMLEntities(c) +  '</text>';
    ...
    if (DEBUG_SHOW_SOURCE) {
        ...
        for (var x = 0; x < grid.width; ++x) {
            for (var y = 0; y < grid.height; ++y) {
                var c = grid(x, y);
                if (c !== ' ') {
                    svg += '<text text-anchor="middle" x="' + (x * SCALE) + '" y="' + (4 + y * SCALE * ASPECT) + '" style="fill:#F00;font-family:Menlo,monaco,monospace;font-size:75%;text-align:center">' + escapeHTMLEntities(c) +  '</text>';
                } // if
            } // y
        } // x
        svg += '</g>';
...
function isGradingOn(str) { return str.search(/\?.*graded.*/i) !== -1; }
function isTaylorialOn(str) { return str.search(/\?.*taylorial.*/i) !== -1; }
...
if (! window.alreadyProcessedMarkdeep) {
    ...
    var gradingOn = isGradingOn(source);
    var taylorialOn = isTaylorialOn(source);
    ...
    var markdeepProcessor = function() {
        ...
        if(taylorialOn) {
            markdeepHTML = TAYLORIAL_HEADER + markdeepHTML + TAYLORIAL_FOOTER;
        }
